  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_pawr_sync_opened_id:
      ...
      sc = sl_bt_sync_update_sync_parameters(
        ...
        sync_timeout);
      app_assert_status(sc);
      // synchronization to sub event 1
    ...
    case sl_bt_evt_pawr_sync_subevent_report_id:
      ...
      if (evt->data.evt_pawr_sync_subevent_report.data.len == 1) {
        led_command = evt->data.evt_pawr_sync_subevent_report.data.data[0];

        app_log("Received data from PAwR advertiser: 0x%02X\r\n", led_command);
        ...
        if (sc != SL_STATUS_OK) {
          ...
          {
            app_log("Sent response with data = %d\r\n", led_command);
          }
      }
      break;
