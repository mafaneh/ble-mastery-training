...
#define TEST_EXT_ELE_LENGTH 252
void demo_setup_ext_adv(uint8_t handle)
{
  ...
  uint8_t manufacturer_specific_data[TEST_EXT_ELE_LENGTH+4];
  ...
  manufacturer_specific_data[0] = TEST_EXT_ELE_LENGTH + 1 + 2;

  // Manufacturer Specific Data
  manufacturer_specific_data[1] = 0xFF;//ad type: manufacturer_specific_data

  // Company ID
  memcpy(&manufacturer_specific_data[2], (uint8_t *)&company_id, 2);

  // Add manufacturer_specific_data
  for (i= 0; i < TEST_EXT_ELE_LENGTH; i++) {
      manufacturer_specific_data[4 + i] = i+1;
  }
  ...
  // Write the data to the system data buffer (needed for chained ext adv)
  sc = sl_bt_system_data_buffer_write(15, adv_data);
  if (sc)
  {
    app_log("result of sl_bt_system_data_buffer_write() is 0x%04X\r\n", (int)sc);
  }

  sc = sl_bt_system_data_buffer_write(4, manufacturer_specific_data);
  {
    app_log("result of sl_bt_system_data_buffer_write() is 0x%04X\r\n", (int)sc);
  }

  sc = sl_bt_system_data_buffer_write(TEST_EXT_ELE_LENGTH, manufacturer_specific_data+4);
  {
    app_log("result of sl_bt_system_data_buffer_write() is 0x%04X\r\n", (int)sc);
  }
  ...
  sc = sl_bt_extended_advertiser_set_long_data(handle);
  app_assert(sc == SL_STATUS_OK,
